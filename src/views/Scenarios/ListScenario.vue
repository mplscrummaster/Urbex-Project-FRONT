...
<template>
  <div class="container">
    <div class="container__filters">
      <button v-for="name in filters" :key="name" class="container__filter" :class="{ active: activeFilter === name }"
        @click.prevent="selectFilter(name)">{{ name }}</button>
    </div>
...
.card {
  ...
  &__bookmark .material-symbols-outlined {
    font-size: 20px;
    line-height: 1;
    color: #888;
  }

  &__bookmark .material-symbols-outlined.fill {
