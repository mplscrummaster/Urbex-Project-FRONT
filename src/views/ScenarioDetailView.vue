  ...
  <div class="p-scenario-detail">
    <div class="p-scenario-detail__header">
      <div class="p-scenario-detail__header-top">
        ...
        <button
          class="p-scenario-detail__bookmark-btn"
          :class="{ 'p-scenario-detail__bookmark-btn--active': isBookmarked }"
        ...
        >
          <span
            ...
            :class="{ 'p-scenario-detail__bookmark-icon--fill': isBookmarked }"
          >
            bookmark
          </span>
        </button>
      ...
      <div class="p-scenario-detail__toolbar">
        <button
          class="p-scenario-detail__play-btn"
          @click="onPlayClick"
          :title="playCtaLabel + ' sur la carte'"
        >
    ...
    <div class="p-scenario-detail__body">
      ...
      <div v-else class="p-scenario-detail__content">
        ...
        <CollapsibleCard
          ...
          :disabled="false"
          variant="intro"
          :completed="scenarioStatus !== 'not_started'"
          :locked="false"
          @toggle="toggleIntro"
        >
          <template #indicator>
            <div
              v-if="scenarioStatus !== 'not_started'"
              class="c-collapsible-card__indicator"
              aria-label="Introduction vue"
              title="Introduction vue"
            >
          ...
          <div class="c-collapsible-card__intro-actions">
            <button
              v-if="scenarioStatus === 'not_started'"
              class="c-collapsible-card__btn c-collapsible-card__btn--primary"
              :disabled="starting"
              @click="startScenario"
        ...
        <div class="p-scenario-detail__missions">
          <ScenarioMissionItem
            ...
            :key="m.id"
            :mission="m"
            :open="openMissions[m.id]"
            :locked="missionLocked(m)"
            :completed="isCompleted(m.id)"
            :scenario-status="scenarioStatus"
            :answer-entry="missionAnswers[m.id] || null"
            :in-range="missionInRange(m)"
            :distance-text="missionDistanceText(m)"
            :lock-reason="missionLockReason(m)"
            @toggle="toggleMission"
            @init-answer="initMissionAnswer"
            @update-answer="
              ({ id, value }) => {
        ...
        <CollapsibleCard
          title="Conclusion"
          :open="showOutro"
          :disabled="!outroUnlocked"
          variant="outro"
          :completed="scenarioStatus === 'completed'"
          :locked="!outroUnlocked"
          @toggle="toggleOutro"
        >
          <template #indicator>
            <div
              v-if="scenarioStatus === 'completed'"
              class="c-collapsible-card__indicator"
              aria-label="Scénario terminé"
          ...
          <div class="c-collapsible-card__outro-actions">
            <button
              ...
              aria-label="Terminer le scénario"
            >
